										...
										throws ParseFailure
	{
		...
		{	
			...
			if (columns == null) {
				...
				Iterator allTables = requestedTableNames.iterator();
				while (allTables.hasNext()) {
					String participant = (String) allTables.next();
					Table participantTable = (Table) tables.get(participant);

					for (int i = 0; i < participantTable.rows().columnCount(); i++) {
						String newColumn = participantTable.rows().columnName(i);
